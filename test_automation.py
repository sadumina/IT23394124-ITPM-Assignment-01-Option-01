...
DEFAULT_TEST_CASE_ID_COLUMN_CANDIDATES = [
    "Test Case ID",
    "TCID",
    "TestCaseID",
    "ID",
...
def _normalize_text_for_assertion(value) -> str:
    if value is None:
        return ""
...
def _build_fallback_output_path(output_path: str) -> str:
    path = Path(output_path)
    stem = f"{path.stem} - results"
    ...
    for i in range(2, 1000):
        ...
        if not candidate.exists():
            return str(candidate)
    return str(path.with_name(f"{stem} - copy{path.suffix}"))

def _save_workbook_with_fallback(wb, output_path: str) -> str:
    try:
        wb.save(output_path)
        return output_path
    except PermissionError:
        fallback = _build_fallback_output_path(output_path)
        wb.save(fallback)
        print(f"Workbook was locked. Saved results to fallback file: {fallback}")
        return fallback
...
def _parse_args():
    ...
    parser.add_argument("--tcid-col", default=None)
...
def run_test():
    ...
    tcid_col_idx = _find_column_index(header_values, args.tcid_col, DEFAULT_TEST_CASE_ID_COLUMN_CANDIDATES)
    ...
    id_prefix = _normalize_test_case_id(args.id_prefix)
    id_regex = re.compile(args.id_regex, re.IGNORECASE) if args.id_regex else None

    if (id_prefix or id_regex) and not tcid_col_idx:
        printable = [str(v) if v is not None else "" for v in header_values]
        print("Error: Could not resolve test case ID column for ID-based filtering.")
        print(f"Header row: {header_row}")
        print(f"Available columns: {printable}")
        return
    ...
    with sync_playwright() as p:
        ...
        for row_index in range(header_row + 1, int(ws.max_row or 0) + 1):
            ...
            tcid_value = (
                _merged_top_left_cell(ws, row_index, tcid_col_idx).value if tcid_col_idx else None
            )
            test_case_id = _normalize_test_case_id(tcid_value)
            if id_prefix and not test_case_id.lower().startswith(id_prefix.lower()):
                continue
            if id_regex and not id_regex.search(test_case_id):
                continue
            ...
            selected += 1
            if args.limit and int(args.limit) > 0 and selected > int(args.limit):
                break
            ...
            row_label = f"{test_case_id} | Row {row_index}" if test_case_id else f"Row {row_index}"
            print(f"Testing [{row_label}]: {singlish_input}")
            ...
            try:
                ...
                if expected_sinhala:
                    status = (
                        "PASS"
                        if _normalize_text_for_assertion(actual_output) == _normalize_text_for_assertion(expected_sinhala)
                        else "FAIL"
                    )
                else:
                    status = "COLLECTED"
                ...
                if args.save_every and int(args.save_every) > 0 and processed % int(args.save_every) == 0:
                    args.output = _save_workbook_with_fallback(wb, args.output)
